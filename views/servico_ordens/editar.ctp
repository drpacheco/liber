...
<?php
print $javascript->link('ordem_servico.js',false);
print $javascript->link('formatar_moeda.js',false);
?>
...
<?php
if ($ajax->isAjax()) {
	print $ajax->form('editar','post',array('autocomplete'=>'off','model'=>'ServicoOrdem','update'=>'conteudo_ajax'));
					...
					<table class="padrao">
						...
						<tbody id="servicos_incluidos">
							...
							foreach ($campos_ja_inseridos as $item) {
								...
								'<td> <input type="text" name="data[ServicoOrdemItem]['.$i.'][valor]" value="'.$geral->numero2moeda($item['valor']).'" class="noinput item_val" /> </td>'.
