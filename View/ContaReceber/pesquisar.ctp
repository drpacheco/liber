					...
					<table class="table table-striped">
						...
						<tbody>
							<?php foreach ($resultados as $r) :
								if (strtoupper($r['ContaReceber']['eh_cliente_ou_fornecedor']) == 'C') $tipo='cliente';
								else $tipo='fornecedor';
								?>
								<tr>
									<td><?php print $r['ContaReceber']['id']; ?></td>
									<td><?php print ucfirst($tipo); ?></td>
									<td>
										<?php
										if ($tipo == 'cliente') print $this->Html->link($r['Cliente']['nome'],'editar/' . $r['ContaReceber']['id']) ;
										else if ($tipo == 'fornecedor') print $this->Html->link($r['Fornecedor']['nome'],'editar/' . $r['ContaReceber']['id']) ;
										?>
									</td>
									<td><?php print $r['ContaReceber']['documento_tipo_id'].' '.$r['DocumentoTipo']['nome']; ?></td>
									<td><?php print $r['ContaReceber']['numero_documento']; ?></td>
									<td><?php print $r['ContaReceber']['valor'] ?></td>
									<td><?php print $r['ContaReceber']['conta_origem'].' '.$r['Conta']['nome'] ?></td>
									<td><?php print $r['ContaReceber']['conta_plano_id'].' '.$r['ContaPlano']['nome'] ; ?></td>
									<td>
										<?php print $this->element('painel_editar',array('id'=>$r['ContaReceber']['id'])) ;?>
									</td>
									<td>
										<?php print $this->element('painel_excluir',array('id'=>$r['ContaReceber']['id'])) ;?>
									</td>
								</tr>
							<?php endforeach; ?>
						</tbody>
