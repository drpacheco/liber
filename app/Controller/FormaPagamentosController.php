...
class FormaPagamentosController extends AppController {
	...
	*/
	function _recupera_itens_inseridos() {
		if (isset($this->request->data['FormaPagamentoItem'])) {
			$itens = $this->request->data['FormaPagamentoItem'];
			$i = 0;
			$valor_total = 0;
	...
	function cadastrar() {
		...
		if (! empty($this->request->data)) {
			...
			if ($this->FormaPagamento->saveAll($this->request->data,array('validate'=>'first'))) {
	...
	function editar($id=NULL) {
		...
		if (empty ($this->request->data)) {
			...
			$this->FormaPagamento->contain('FormaPagamentoItem');
			$this->request->data = $this->FormaPagamento->read();
			...
			$this->_recupera_itens_inseridos();
			$this->request->data['FormaPagamento']['id'] = $id;
