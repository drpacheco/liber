...
<?php echo "<?php echo \$this->Form->create('{$modelClass}');?>\n";?>
	<fieldset>
 		<legend><?php echo "<?php printf(__('" . $actionTitulo . " %s'), __('" . Inflexao::acentos($singularHumanName) . "')); ?>";?></legend>
<?php
		echo "\t<?php\n";
...
<?php if (strpos($action, 'add') === false): ?>
		<li><?php echo "<?php echo \$this->Html->link(__('Excluir'), array('action' => 'delete', \$this->Form->value('{$modelClass}.{$primaryKey}')), null, sprintf(__('Você tem certeza que deseja excluir o id #%s?'), \$this->Form->value('{$modelClass}.{$primaryKey}'))); ?>";?></li>
<?php endif;?>
		<li><?php echo "<?php echo \$this->Html->link(sprintf(__('Listar %s'), __('" . Inflexao::acentos($pluralHumanName) . "')), array('action' => 'index'));?>";?></li>
<?php
		$done = array();
