...
<?php
if ($this->Ajax->isAjax()) {
	print $this->Ajax->form('editar','post',array('autocomplete'=>'off','model'=>'ServicoOrdem','update'=>'conteudo_ajax'));
					...
					<table class="padrao">
						...
						<tbody id="servicos_incluidos">
							...
							foreach ($campos_ja_inseridos as $item) {
								...
								'<td> <input type="text" name="data[ServicoOrdemItem]['.$i.'][valor]" value="'.$this->Geral->numero2moeda($item['valor']).'" class="noinput item_val" /> </td>'.
